...
DEFAULT_DET_PATH = "configs/det/ch_PP-OCRv3/ch_PP-OCRv3_det.yml"
DEFAULT_REC_PATH = "configs/rec/PP-OCRv3/ch_PP-OCRv3_rec.yml"
DEFAULT_CLS_PATH = "configs/cls/cls_mv3.yml"
DEFAULT_CFG_PATH = "configs/rapidocr/config.yaml"
class RapidOCR:
    def __init__(self,  device_id=0, **kwargs):
        ...
        self.text_det = TextDetector(DEFAULT_DET_PATH, **{"device_id":device_id,
                                                          "model_path":config['Det']['model_path'],
                                                          "use_cuda":config['Det']['use_cuda']})

        self.use_cls = global_config['use_cls']
        self.text_cls = TextClassifier(DEFAULT_CLS_PATH, **{"device_id":device_id,
                                                            "model_path":config['Cls']['model_path'],
                                                            "cls_image_shape":config['Cls']['cls_image_shape'],
                                                            "cls_batch_num":config['Cls']['cls_batch_num'],
                                                            "cls_thresh":config['Cls']['cls_thresh'],
                                                            "label_list":config['Cls']['label_list'],
                                                            "use_cuda":config['Cls']['use_cuda']})

        self.use_rec = global_config['use_rec']
        self.text_rec = TextRecognizer(DEFAULT_REC_PATH, **{"device_id":device_id,
                                                            "model_path":config['Rec']['model_path'],
                                                            "use_cuda":config['Rec']['use_cuda']})
    ...
    def get_crop_img_list(
        ...
        for box in dt_boxes:
            ...
            # 保存裁剪后的图像到指定文件夹
            crop_img_path = os.path.join("new_check", f"crop_img_" + str(box) + ".jpg")
...
def main():
    ...
    result, elapse_list = ocr_engine(
        ...
        box_thresh= 0.5,
        unclip_ratio=1.6,
        text_score=0.4 , det_use_cuda=True, cls_use_cuda=True, rec_use_cuda=True
    )
    logger.info(result)
